...
// Database row shape — minimal typing, kelajakda kengaytirsa bo'ladi
...
export default function WarehousePage() {
  ...
  // Auth — RBAC source of truth
  const { role: userRole, canEdit, canDelete } = useAuth();
  ...
  useEffect(() => {
    ...
    const savedRate = window.localStorage.getItem('exchangeRate');
    if (savedRate) {
  ...
  const handleSaveRate = () => {
    ...
    if (r > 0) {
      setExchangeRate(r);
      window.localStorage.setItem('exchangeRate', r.toString());
  ...
  const [spisReason, setSpisReason] = useState('Brak / Shikastlangan');
  ...
  // Tarix
  const [historyDocs, setHistoryDocs] = useState<Row[]>([]);
    ...
    try {
      ...
      if (activeTab === 'qoldiq') {
        const { data } = await supabase
          ...
          .select('*, products(name, image_url, categories(name))');
        ...
        if (data) {
          ...
          let enrichedBalances: Row[] = data.map((b: Row) => {
            const pTxs = txs?.filter((t: Row) => t.product_id === b.product_id) || [];

            const thirtyDaySales = pTxs
              .filter(
                (t: Row) =>
                  t.transaction_type !== 'kirim' &&
                  new Date(t.created_at) >= thirtyDaysAgo
              )
              .reduce((sum: number, t: Row) => sum + Math.abs(t.quantity_change), 0);
            const minStock = Math.ceil((thirtyDaySales / 30) * 2) || 0;

            const rangeTxs = pTxs.filter((t: Row) => {
          ...
          enrichedBalances.sort((a: Row, b: Row) => b.totalValue - a.totalValue);
          const grandTotal = enrichedBalances.reduce(
            (sum: number, b: Row) => sum + b.totalValue,
            0
          );
          let runningSum = 0;

          enrichedBalances = enrichedBalances.map((b: Row) => {
      ...
      setFetchError('Ulanish xatosi: ' + message);
    }
  ...
  const handleDeleteReceipt = async (
    ...
    if (!confirm("Rostdan ham bu hujjatni o'chirmoqchimisiz? Ombor qoldig'i orqaga qaytariladi!"))
      return;
    try {
      ...
      if (sup) {
        await supabase
          .from('suppliers')
          .update({ balance: Number(sup.balance) - totalAmount })
          .eq('id', supplierId);
      }
      for (const item of items) {
        ...
        if (bal) {
          await supabase
            .from('inventory_balances')
            .update({ quantity: bal.quantity - item.quantity })
            .eq('product_id', item.product_id);
        }
      }
  ...
  const handleEditReceipt = async (doc: Row) => {
    if (
      !confirm(
        "Tahrirlash uchun eski hujjat bazadan o'chirilib, ma'lumotlari 'Yangi Kirim' oynasiga ko'chiriladi. Rozimisiz?"
      )
    )
      return;
    try {
      ...
      if (sup) {
        await supabase
          .from('suppliers')
          .update({ balance: Number(sup.balance) - doc.total_amount })
          .eq('id', doc.supplier_id);
      }
      ...
      fetchData();
      alert(
        "Tahrirlashga tayyor! O'zgartirishlarni kiritgach pastdagi 'Tasdiqlash' tugmasini bosib yangidan saqlang."
      );
    } catch (err: unknown) {
      alert('Xatolik: ' + (err instanceof Error ? err.message : String(err)));
    }
  ...
  const saveSupplierTerm = async (id: string) => {
    try {
      const { error } = await supabase
        .from('suppliers')
        .update({ payment_term_days: Number(tempDays) })
        .eq('id', id);
  ...
  const handleAddSupplier = async () => {
    ...
    try {
      const { error } = await supabase
        .from('suppliers')
        .insert([{ name: newSupplierName, phone: newSupplierPhone }]);
      if (error) throw error;
      setNewSupplierName('');
      setNewSupplierPhone('');
      fetchData();
  ...
  const handleAddProduct = async () => {
    if (!productName || !categoryId || !imageFile) {
      alert('Nomini, Kategoriyani va Rasmni kiritish MAJBURIY!');
      return;
    }
    setLoading(true);
    setSuccessMsg('');
    try {
      ...
      const { error } = await supabase
        .from('products')
        .insert([{ name: productName, category_id: categoryId, image_url: imageUrl }]);
      if (error) throw error;
  ...
  const handleDeleteProduct = async (id: string) => {
    ...
    try {
      ...
      alert("O'chirishda xato: " + (err instanceof Error ? err.message : String(err)));
    }
  };

  const handleUpdateProduct = async (id: string, oldImageUrl: string) => {
    if (!editProdName || !editProdCat) {
      alert('Nom va kategoriyani kiriting!');
      return;
    }
    setLoading(true);
    try {
      ...
      const { error } = await supabase
        .from('products')
        .update({ name: editProdName, category_id: editProdCat, image_url: newImageUrl })
        .eq('id', id);
      if (error) throw error;
      ...
      alert('Yangilashda xato: ' + (err instanceof Error ? err.message : String(err)));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddItemToReceipt = () => {
    ...
    setReceiptItems([
      ...receiptItems,
      {
        ...
        quantity: Number(kirimQty),
        incoming_price: Number(kirimPriceUsd),
        total: Number(kirimQty) * Number(kirimPriceUsd),
      },
    ]);
    setSelectedProduct('');
    setKirimQty('');
  ...
  const handleSubmitReceipt = async () => {
    if (!selectedSupplier) {
      alert('Xato: Postavshik (Hamkor) tanlash majburiy!');
      return;
    }
    if (receiptItems.length === 0) {
      alert("Xato: Hech qanday tovar qo'shilmagan!");
      return;
    }
    ...
    try {
      ...
      const { data: supTerm } = await supabase
        .from('suppliers')
      ...
      const { data: receipt, error: receiptError } = await supabase
        .from('receipt_docs')
        .insert([
          { supplier_id: selectedSupplier, total_amount: totalAmountUsd, due_date: dueDate },
        ])
        .select()
        .single();
      ...
      for (const item of receiptItems) {
        ...
        if (bal) {
          ...
          const newAvgPrice =
            (bal.quantity * bal.average_price + item.quantity * item.incoming_price) / newQty;
          await supabase
            .from('inventory_balances')
            .update({ quantity: newQty, average_price: newAvgPrice, updated_at: new Date().toISOString() })
            .eq('product_id', item.product_id);
        } else {
          await supabase.from('inventory_balances').insert([
            {
              product_id: item.product_id,
              quantity: item.quantity,
              average_price: item.incoming_price,
            },
          ]);
        }
      ...
      if (sup) {
        await supabase
          .from('suppliers')
          .update({ balance: Number(sup.balance) + totalAmountUsd })
          .eq('id', selectedSupplier);
      }
      setKirimSuccess('Kirim hujjati saqlandi!');
      setReceiptItems([]);
      setSelectedSupplier('');
    } catch (err: unknown) {
      alert('Xatolik: ' + (err instanceof Error ? err.message : String(err)));
  ...
  const handleSpisaniyaSubmit = async () => {
    ...
    try {
      const { data: bal } = await supabase
        .from('inventory_balances')
        .select('quantity')
        .eq('product_id', spisProduct)
        .single();
      if (!bal || bal.quantity < Number(spisQty)) {
        alert(`Yetersiz qoldiq! Mavjud: ${bal?.quantity || 0} ta`);
      ...
      if (!batches || batches.length === 0) {
        alert('Tovar partiyalari topilmadi!');
        setSpisLoading(false);
        return;
      }
      ...
      if (qtyToDeduct > 0) {
        alert('Xato: FIFO qoldiq yetarli emas!');
        setSpisLoading(false);
        return;
      }
      ...
      const { data: wo } = await supabase
        .from('write_offs')
        .insert([{ reason: spisReason, total_value: totalCost }])
        .select()
        .single();
      await supabase.from('write_off_items').insert([
        {
          write_off_id: wo?.id,
          product_id: spisProduct,
          quantity: Number(spisQty),
          cost_value: totalCost,
        },
      ]);
      await supabase
        .from('inventory_balances')
        .update({ quantity: bal.quantity - Number(spisQty), updated_at: new Date().toISOString() })
        .eq('product_id', spisProduct);
      await supabase.from('inventory_transactions').insert([
        {
          product_id: spisProduct,
          transaction_type: 'spisaniya',
          quantity_change: -Number(spisQty),
          price: totalCost / Number(spisQty),
          reference_id: wo?.id,
        },
      ]);
  ...
  const allTabs = ['katalog', 'kirim', 'spisaniya', 'tarix', 'qoldiq', 'hamkorlar'];
  const visibleTabs = allTabs.filter(
    (tab) => !(userRole === 'skladchi' && tab === 'spisaniya')
  );
    ...
    <div>
        ...
        >
          ...
          {isEditingRate && canEdit ? (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span>
                1$ = <b>{exchangeRate.toLocaleString()} UZS</b>
              </span>
              {canEdit && (
                <button
        ...
        {visibleTabs.map((tab) => (
          ...
          >
            ...
            {tab === 'katalog'
              ...
              ? 'Kirim (Nakladnoy)'
              : tab === 'spisaniya'
              ? 'Hisobdan Chiqarish (Spisaniya)'
              : tab === 'qoldiq'
              ? "Ombor Qoldig'i (Analitika)"
              : tab}
          </button>
        ))}
      </div>
          ...
          <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid var(--border)' }}>
                ...
                {(canEdit || canDelete) && (
                  <th style={{ padding: '12px 0', textAlign: 'right' }}>Harakatlar</th>
                )}
              </tr>
            </thead>
            <tbody>
                    ...
                    <td style={{ padding: '12px 0', textAlign: 'right' }}>
                      {editingProductId === prod.id ? (
                        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                          <button
                            onClick={() => handleUpdateProduct(prod.id, prod.image_url)}
                            className="btn btn-primary"
                          ...
                          {canEdit && (
                            <button
                              onClick={() => {
                                ...
                                setEditProdCat(prod.category_id);
                              }}
                              className="btn"
                              style={{
                                padding: '4px 8px',
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: '16px', marginBottom: '16px', flexWrap: 'wrap' }}>
            ...
            <div style={{ position: 'relative', flex: 2, minWidth: '220px' }}>
              ...
              {showKirimDropdown && (
                ...
                >
                  {products
                    .filter((p) =>
                      p.name.toLowerCase().includes(kirimSearchTerm.toLowerCase())
                    )
                    .map((prod) => (
                      <div
                        key={prod.id}
            ...
            <input
              ...
              placeholder="Soni"
              value={kirimQty}
              onChange={(e) => setKirimQty(e.target.value)}
              style={{
                width: '80px',
                padding: '8px',
                border: '1px solid var(--border)',
            ...
            <input
              ...
              placeholder="Narx ($)"
              value={kirimPriceUsd}
              onChange={(e) => setKirimPriceUsd(e.target.value)}
              style={{
                width: '100px',
                padding: '8px',
                border: '1px solid var(--border)',
            ...
            />
            <button onClick={handleAddItemToReceipt} className="btn">
              Qo&apos;shish
            </button>
          </div>
          ...
          <button
            ...
            disabled={kirimLoading || receiptItems.length === 0}
          >
            {kirimLoading ? 'Saqlanmoqda...' : 'Tasdiqlash'}
          </button>
        </div>
      )}
      ...
      {activeTab === 'spisaniya' && (
        <div className="card">
          <h2 style={{ marginBottom: '16px', fontSize: '1.2rem' }}>
            Tovarlarni Hisobdan Chiqarish (Spisaniya)
          </h2>
          {spisSuccess && (
            <div
              style={{
          ...
          >
              ...
              <div style={{ position: 'relative' }}>
                ...
                {showSpisDropdown && (
                  ...
                  >
                    {products
                      .filter((p) =>
                        p.name.toLowerCase().includes(spisSearchTerm.toLowerCase())
                      )
                      .map((prod) => (
                        <div
                          key={prod.id}
                          onClick={() => {
                            setSpisProduct(prod.id);
                            setSpisSearchTerm(prod.name);
                            setShowSpisDropdown(false);
                          }}
                          style={{
                            padding: '10px',
                            cursor: 'pointer',
            ...
            <div>
              ...
              <input
                type="number"
                value={spisQty}
                onChange={(e) => setSpisQty(e.target.value)}
                placeholder="0"
                style={{
                  width: '100%',
                  padding: '10px',
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              ...
              <select
                value={spisReason}
                onChange={(e) => setSpisReason(e.target.value)}
                style={{
                  width: '100%',
                  padding: '10px',
                  borderRadius: '4px',
          ...
          <button
            className="btn btn-primary"
            onClick={handleSpisaniyaSubmit}
            disabled={spisLoading}
      ...
      {activeTab === 'qoldiq' && (
        <div className="card" style={{ maxWidth: '100%', overflowX: 'auto' }}>
          ...
          >
            <div>
              <h2 style={{ fontSize: '1.3rem', margin: 0, marginBottom: '8px' }}>
                Ombor Qoldig&apos;i va Analitika
              </h2>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', margin: 0 }}>
                Har bir tovar bo&apos;yicha kirim, chiqim va joriy qoldiq tahlili
              </p>
            </div>
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <span
                  style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 'bold' }}
                >
                  Boshlanish Sana
                </span>
                <input
          ...
          >
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '12px 8px', borderLeft: '1px solid #e2e8f0' }}>
                      ...
                      <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                        ${bal.chiqimValue.toLocaleString()}
                      </div>
                    </td>
            ...
            <tfoot>
              <tr style={{ backgroundColor: '#f1f5f9' }}>
                <td colSpan={2} style={{ padding: '16px 8px', textAlign: 'right', fontWeight: 'bold' }}>
                  JAMI (Tanlangan Davr):
                </td>
                <td
                  style={{
                ...
                >
                  ...
                  <span style={{ fontSize: '0.9rem', color: '#0f172a' }}>
                    ${balances.reduce((s, b) => s + b.kirimValue, 0).toLocaleString()}
                  </span>
                </td>
                <td
                  style={{
                ...
                >
                  ...
                  <span style={{ fontSize: '0.9rem', color: '#0f172a' }}>
                    ${balances.reduce((s, b) => s + b.chiqimValue, 0).toLocaleString()}
                  </span>
                </td>
                <td
                  style={{
        ...
        <div className="card" style={{ backgroundColor: '#f8fafc', padding: '24px' }}>
          ...
          >
            <h2 style={{ fontSize: '1.4rem', margin: 0, color: '#0f172a' }}>
              Kirim Hujjatlari (Nakladnoylar)
            </h2>
            <div style={{ fontSize: '0.9rem', color: '#64748b' }}>
              Jami {historyDocs.length} ta hujjat
            </div>
          </div>
          ...
          {historyDocs.length === 0 ? (
            ...
            >
              <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem' }}>
                Hozircha hech qanday kirim hujjati yo&apos;q.
              </p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div style={{ padding: '20px 24px' }}>
                    <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
                      ...
                      <tbody>
                        {doc.receipt_items?.map((item: Row) => (
                          <tr key={item.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                            ...
                            <td style={{ padding: '12px', color: '#475569' }}>
                              ${item.incoming_price?.toLocaleString()}
                            </td>
                            <td
                              style={{
                                padding: '12px',
      ...
      {activeTab === 'hamkorlar' && (
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: '16px', marginBottom: '32px', flexWrap: 'wrap' }}>
            <input
              type="text"
              placeholder="Hamkor (Postavshik) nomi"
            ...
            />
            <button onClick={handleAddSupplier} disabled={supplierLoading} className="btn btn-primary">
              {supplierLoading ? 'Saqlanmoqda...' : "Qo'shish"}
            </button>
          </div>

          <h2 style={{ marginBottom: '16px', fontSize: '1.2rem' }}>
            Hamkorlar bilan ishlash muddati
          </h2>
          <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid var(--border)' }}>
                ...
                {canEdit && <th style={{ padding: '12px 0' }}>Harakat</th>}
              </tr>
            </thead>
            <tbody>
              {suppliers.map((sup) => (
                <tr key={sup.id} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td
                    style={{
                      padding: '12px 0',
                      color: sup.balance > 0 ? '#b91c1c' : '#15803d',
                      fontWeight: 'bold',
                    }}
                  >
                    ${Number(sup.balance).toLocaleString()}
                  </td>
                  <td style={{ padding: '12px 0' }}>
                  ...
                  {canEdit && (
                    <td style={{ padding: '12px 0' }}>
                      {editingSupplier === sup.id ? (
                        <button
                          className="btn btn-primary"
                          onClick={() => saveSupplierTerm(sup.id)}
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setTempDays(String(sup.payment_term_days || 0));
                          }}
                          style={{
                            padding: '6px 12px',
                            fontSize: '0.9rem',
